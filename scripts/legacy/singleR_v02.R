...
save(singleR_result_AxB, file = "singleR_AxB.RData")
save(df_long_AxB, file = "df_long_AxB.RData")
...
setwd("~/Google Drive/My Drive/Gensel lab/R studies/SingleR/")
save(singleR_result_AxC, file = "singleR_AxC.RData")
save(df_long_AxC, file = "df_long_AxC.RData")
...
setwd("~/Google Drive/My Drive/Gensel lab/R studies/SingleR/")
save(singleR_result_BxC, file = "singleR_BxC.RData")
save(df_long_BxC, file = "df_long_BxC.RData")
...
heatmap1 <- ggplot(df_long_AxB, aes(x = predicted_cell_types, y = query_clusters, fill = value)) +
  geom_tile(color = "white") +
  fill_scale +
  labs(title = "Dataset A vs Dataset B",
       x = "Reference Clusters (Dataset A)", 
       y = "Query Clusters (Dataset B)",
...
heatmap2 <- ggplot(df_long_AxC, aes(x = predicted_cell_types, y = query_clusters, fill = value)) +
  geom_tile(color = "white") +
  fill_scale +
  labs(title = "Dataset A vs Dataset C",
       x = "Reference Clusters (Dataset A)", 
       y = "Query Clusters (Dataset C)",
       fill = "Number of cells\npredicted on\nreference clusters") +
  theme_minimal() +
...
heatmap3 <- ggplot(df_long_BxC, aes(x = predicted_cell_types, y = query_clusters, fill = value)) +
  geom_tile(color = "white") +
  fill_scale +
  labs(title = "Dataset B vs Dataset C",
       x = "Reference Clusters (Dataset B)", 
       y = "Query Clusters (Dataset C)",
       fill = "Number of cells\npredicted on\nreference clusters") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
...
final_plot
ggsave("SingleR_all.png", dpi = 300, bg = "white")
